...
public class PizzaShopService {
    ...
    public List<Pizza> getPizzas() {
        List<Pizza> pizza_list = new ArrayList<>();

        for (DamenosPizza damenos: damenosService.getPizzas()) {
            pizza_list.add(new DamenosAdapter(damenos));
        };

        for (PizzaHotPizza pizzaHot: pizzaHotService.getPizzas()) {
            pizza_list.add(new PizzahotAdapter(pizzaHot));
        };
